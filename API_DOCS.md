...
| GET | `/candidate-invitations/validate/{token}` | Public | Validates token unless expired |
...
| GET | `/questions` | `ADMIN`, `SUPERADMIN` | Non-superadmin sees `PUBLIC` + own `ORG_OWNED`; optional `subjectId`, `topicId`, `subtopicId` filters |
...
| GET | `/test-questions/{id}` | JWT | Existing route remains; service-level hardening should be treated as follow-up if not yet completed |
| GET | `/test-questions` | JWT | Existing route remains |
| GET | `/test-questions/test/{testId}` | JWT | Existing route remains |
| GET | `/test-questions/test/{testId}/grouped` | JWT | Existing route remains |
| PUT | `/test-questions/{id}` | JWT | Existing route remains |
| PATCH | `/test-questions/{id}` | JWT | Existing route remains |
| DELETE | `/test-questions/{id}` | `ADMIN`, `SUPERADMIN` | Existing route remains |
...
| Method | Path | Access | Notes |
|---|---|---|---|
| POST | `/test-cases` | JWT | Existing route remains |
| GET | `/test-cases` | JWT | Existing route remains |
| GET | `/test-cases/{id}` | JWT | Existing route remains |
| PATCH | `/test-cases/update/{id}` | JWT | Existing route remains |
| DELETE | `/test-cases/delete/{id}` | JWT | Existing route remains |
...
- only `SUPERADMIN` should send `PUBLIC`
...
- coding submissions stay `PENDING` for async grading
...
- `TestQuestion` and `TestCase` routes still need the same full tenant hardening pass if you want the docs to claim strict resource-level guarantees there as well.
