...
export default function NewAdminQuestionCreate() {
  ...
  const handleAutoGenerateTemplates = async () => {
    setIsGeneratingTemplates(true);
    try {
      const generated = await testService.getAutoGeneratedTemplates(signature as any);
      if (generated) {
        setCodeTemplates((prev) => ({
          python3: {
            template: generated.python?.template || prev.python3.template,
            driver: generated.python?.driver || prev.python3.driver,
          },
          javascript: {
            template: generated.javascript?.template || prev.javascript.template,
            driver: generated.javascript?.driver || prev.javascript.driver,
          },
          java: {
            template: generated.java?.template || prev.java.template,
            driver: generated.java?.driver || prev.java.driver,
          },
          cpp: {
            template: generated.cpp?.template || prev.cpp.template,
            driver: generated.cpp?.driver || prev.cpp.driver,
          },
        }));
        toast.success("Templates & drivers generated successfully");
      }
    } catch {
      toast.info("Generated baseline boilerplate for selected signature");
    } finally {
            ...
            {isCoding ? (
              ...
              <div className="space-y-8 pt-4 border-t border-slate-100">
                ...
                <div className="space-y-4 pt-4 border-t border-slate-100">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="text-xs font-bold text-slate-800 uppercase tracking-wider">Method Signature Configuration</h4>
                      <p className="text-[11px] text-slate-400">Define function signature for automatic multi-language code generation.</p>
                    </div>
                    <button
                      type="button"
                      onClick={handleAutoGenerateTemplates}
                      disabled={isGeneratingTemplates}
                      className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-800 border border-slate-300 text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer"
                    >
                      {isGeneratingTemplates ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Sparkles className="w-3.5 h-3.5 text-slate-600" />}
                  ...
                  <div className="space-y-2 pt-1">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-semibold text-slate-700">Parameters</span>
                      <button
                        type="button"
                        onClick={addParam}
                      ...
                      {signature.params.map((p, idx) => (
                        <div key={idx} className="flex items-center gap-4">
                          <input
                            ...
                            className="flex-1 border-b border-slate-200 focus:border-[#4353a4] py-1 text-xs text-slate-800 font-mono focus:outline-none bg-transparent"
                          />
                          <input
                            type="text"
                            value={p.type}
                            ...
                            className="w-44 border-b border-slate-200 focus:border-[#4353a4] py-1 text-xs text-slate-800 font-mono focus:outline-none bg-transparent"
                          />
                ...
                <div className="space-y-4 pt-4 border-t border-slate-100">
                  <div>
                    <h4 className="text-xs font-bold text-slate-800 uppercase tracking-wider">Code Templates & Drivers</h4>
                    <p className="text-[11px] text-slate-400">Set candidate-facing starter code and hidden execution drivers per language.</p>
                  </div>
                  ...
                  <div className="flex border-b border-slate-200">
                    {(
                      ...
                      <button
                        ...
                        className={`px-4 py-2 text-xs font-semibold transition-colors border-b-2 -mb-[2px] cursor-pointer ${
                          activeCodeLang === lang.id
                            ? "border-[#4353a4] text-[#4353a4]"
                  ...
                  <div className="space-y-4 pt-2">
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center justify-between">
                        <label className="block text-xs font-semibold text-slate-700">Starter Code Template (Candidate Facing)</label>
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center justify-between">
                        <label className="block text-xs font-semibold text-slate-700">Execution Driver (Hidden / Admin Only)</label>
                        <span className="text-[10px] text-slate-400 font-mono">Appended during compilation</span>
                      </div>
                      <textarea
                        rows={4}
                        ...
                        onChange={(e) =>
                          setCodeTemplates({
                            ...codeTemplates,
                            [activeCodeLang]: { ...codeTemplates[activeCodeLang], driver: e.target.value },
                          })
                        }
                        placeholder="// Optional runner / assertion code"
                        className="w-full border border-slate-200 p-3 text-xs font-mono text-slate-800 bg-slate-50/40 focus:bg-white focus:outline-none focus:border-[#4353a4]"
                      />
                    </div>
                  </div>
