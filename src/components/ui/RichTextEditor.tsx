...
export const RichTextEditor: React.FC<RichTextEditorProps> = ({
  ...
  const extensions = React.useMemo(
    () => [
      ...
      Link.configure({
        openOnClick: false,
        HTMLAttributes: {
          class: "text-indigo-600 underline font-medium cursor-pointer",
          target: "_blank",
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: compact
          ? `prose prose-slate prose-sm max-w-none focus:outline-none px-3 py-1.5 text-slate-800 text-xs leading-normal bg-white font-sans [&_p]:my-0.5 [&_code]:font-mono [&_code]:text-[11px] [&_code]:bg-slate-100 [&_code]:text-pink-600 [&_code]:px-1 [&_code]:py-0.5 [&_code]:rounded-xs [&_a]:text-indigo-600`
          : `prose prose-slate prose-sm max-w-none focus:outline-none px-3.5 py-3 text-slate-800 text-xs leading-relaxed bg-white font-sans [&_ol]:list-decimal [&_ol]:pl-5 [&_ul]:list-disc [&_ul]:pl-5 [&_li]:my-1 [&_p]:my-1 [&_pre]:bg-[#1e1e1e] [&_pre]:text-[#d4d4d4] [&_pre]:border [&_pre]:border-slate-700 [&_pre]:p-3.5 [&_pre]:rounded-md [&_pre]:font-mono [&_pre]:text-[12px] [&_pre]:leading-relaxed [&_pre]:my-2 [&_pre]:shadow-md [&_code]:font-mono [&_code]:text-[11px] [&_code]:bg-slate-100 [&_code]:text-pink-600 [&_code]:px-1 [&_code]:py-0.5 [&_code]:rounded-xs [&_pre_code]:bg-transparent [&_pre_code]:text-inherit [&_pre_code]:p-0 [&_a]:text-indigo-600 [&_a]:underline [&_a]:font-medium [&_blockquote]:border-none [&_blockquote]:italic [&_blockquote]:text-slate-700 [&_blockquote]:my-1.5 [&_blockquote]:px-1 [&_blockquote]:before:content-['“'] [&_blockquote]:after:content-['”'] [&_blockquote]:before:font-serif [&_blockquote]:after:font-serif [&_blockquote]:before:text-indigo-600 [&_blockquote]:after:text-indigo-600 [&_blockquote]:before:font-bold [&_blockquote]:after:font-bold`,
  ...
  return (
    <>
      <div className={`border border-slate-200 rounded-xs overflow-hidden bg-white flex flex-col shadow-xs focus-within:border-indigo-600 transition-colors ${className}`}>
        {/* Toolbar */}
        {compact ? (
        ...
        <DialogContent className="max-w-md bg-white border border-slate-200 rounded-xs shadow-lg p-0 overflow-hidden font-sans">
          ...
          >
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-stretch border border-slate-200 focus-within:border-indigo-600 transition-colors rounded-xs overflow-hidden">
            ...
            {isSelectionCollapsed && (
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full border border-slate-200 px-3 py-2 text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:border-indigo-600 rounded-xs"
                />
              </div>
            )}
            ...
            <DialogFooter className="pt-2 border-t border-slate-100 flex items-center justify-between sm:justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  type="submit"
                  className="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold uppercase tracking-wider rounded-xs shadow-xs transition-colors cursor-pointer"
                >
